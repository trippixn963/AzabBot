...
def create_activate_command(bot):
    ...
    @app_commands.command(
        name="activate", description="Bot is always online - use /status instead"
    )
...
def create_deactivate_command(bot):
    ...
    @app_commands.command(
        name="deactivate", description="Bot is always online - cannot be deactivated"
    )
...
def create_status_command(bot):
    ...
    @app_commands.command(
        name="status", description="Check bot status (Developer only)"
    )
    async def status(interaction: discord.Interaction):
        ...
        embed = EmbedBuilder.create_status_embed(
            ...
            stats={
                "prisoners": len(bot.current_prisoners)
                if hasattr(bot, "current_prisoners")
                else 0,
