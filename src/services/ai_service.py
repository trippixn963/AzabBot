...
from src.services.personality_service import PersonalityService, PersonalityMode
        ...
        try:
            ...
            if mode == ResponseMode.AZAB:
                ...
                if context.additional_context.get('batch_size', 1) > 1:
                    combined_msg = f"The user sent multiple messages:\n{context.message_content}"
...
class AIService(BaseService):
    ...
    def __init__(self, name: str = "AIService"):
        ...
        super().__init__(name, dependencies=["PrisonerDatabaseService", "PersonalityService", "MemoryService"])
        ...
        try:
            ...
            if self.memory_service:
                try:
                    ...
                    actual_user_id = additional_context.get('user_id', channel_id) if additional_context else channel_id
                    ...
                    self.logger.log_info(f"DEBUG: Retrieved user context for {user_name} (ID: {actual_user_id})")
                except Exception as e:
                    self.logger.log_warning(
                        f"Memory service error: {e}",
                        extra={"user_id": actual_user_id, "error_type": type(e).__name__}
                    )
            ...
            if self.personality_service:
                try:
                    ...
                    personality_prompt = self.personality_service.get_personality_prompt(personality_mode)
                    self.logger.log_info(
                        f"✅ Selected personality: {personality_mode.value} for {user_name}",
                        extra={"personality": personality_mode.value, "user": user_name}
                    )
                except Exception as e:
                    self.logger.log_warning(
            ...
            if self.personality_service and personality_mode:
                try:
                    ...
                    self.logger.log_warning(f"Failed to track personality effectiveness: {e}")
            ...
            if self.memory_service and user_memory:
                try:
                    ...
                    self.memory_service.update_response_effectiveness(
                        user_id=actual_user_id,
                        strategy=personality_mode.value if personality_mode else response_mode.value,
