...
class TestRateLimiter:
    ...
    def test_rate_limiter_time_window(self):
        ...
        limiter = RateLimiter()
        config = RateLimitConfig(max_requests=1, time_window=1)  # 1 req per second
...
class TestSecurityValidator:
    ...
    def test_validate_discord_id(self):
        ...
        # Valid ID
        assert validator.validate_discord_id("123456789012345678") == 123456789012345678
        assert validator.validate_discord_id(123456789012345678) == 123456789012345678
    ...
    def test_validate_permission_level(self):
        ...
        # Valid levels
        assert validator.validate_permission_level("admin") == PermissionLevel.ADMIN
        assert validator.validate_permission_level("USER") == PermissionLevel.USER
...
class TestAccessController:
    ...
    def test_role_permissions(self):
        ...
        assert controller.get_user_permission_level(context) == PermissionLevel.MODERATOR
        
        # Test higher role takes precedence
        context = SecurityContext(user_id=2, roles=[100, 200])
        assert controller.get_user_permission_level(context) == PermissionLevel.ADMIN
    ...
    def test_feature_permissions(self):
        ...
        assert controller.check_permission(context, "moderate_channels") is True
    ...
    def test_require_permission(self):
        ...
        controller = AccessController()
        context = SecurityContext(user_id=1, permission_level=PermissionLevel.USER)
...
class TestSecurityManager:
    ...
    def test_check_interaction_security_success(self):
        ...
        # Set up permissions
        manager.access_controller.set_role_permission(100, PermissionLevel.USER)
        
        context = manager.create_security_context(user_id=123, roles=[100])
        
        # Should succeed
        assert manager.check_interaction_security(
            context, "use_bot", "Hello bot"
        ) is True
