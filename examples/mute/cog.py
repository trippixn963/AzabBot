...
class MuteCog(HelpersMixin, AutocompleteMixin, MuteOpsMixin, UnmuteOpsMixin, commands.Cog):
    ...
    @app_commands.describe(
        ...
        buyout="Allow user to buy out of mute (default: True)",
    )
    ...
    async def mute(
        ...
        await self.execute_mute(interaction, user, duration, reason, evidence_result.url, buyout_allowed=buyout)
    ...
    async def unmute(
        ...
        # Defer and let execute_unmute handle all validation (including absent users)
        try:
            if not interaction.response.is_done():
                await interaction.response.defer(ephemeral=False)
        except discord.HTTPException:
