    ...
    def _init_tables(self: "DatabaseManager") -> None:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS active_mutes (
                ...
                buyout_allowed INTEGER DEFAULT 1,
        ...
        # Migration: Add buyout_allowed column if missing
        try:
            cursor.execute("ALTER TABLE active_mutes ADD COLUMN buyout_allowed INTEGER DEFAULT 1")
        except sqlite3.OperationalError:
            pass  # Column already exists
        # Migration: Add remaining_seconds for paused mutes (user left while muted)
        try:
            cursor.execute("ALTER TABLE active_mutes ADD COLUMN remaining_seconds REAL")
        except sqlite3.OperationalError:
            pass  # Column already exists
        ...
        """)
        for col in ["expires_at REAL", "case_id TEXT", "remaining_seconds REAL"]:
            try:
        ...
        for col in [
            ...
            "transcript_token TEXT",  # Public access token for transcript viewer
            "description TEXT",  # User-provided ticket description
            "answers_json TEXT",  # Raw modal answers as JSON {label: value}
        ]:
            try:
        ...
        # NOTE: message_samples, user_activity_hours, user_interactions tables
        # were removed — they were created for alt detection but never used.
