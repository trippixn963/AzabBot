...
class MutesMixin:
    ...
    def add_mute(
        ...
        Args:
            ...
            buyout_allowed: Whether the user can buy out of the mute.
        ...
        # Atomic: both active_mutes and mute_history must succeed together
        with self.transaction() as tx:
            tx.execute(
                """INSERT OR REPLACE INTO active_mutes
                   (user_id, guild_id, moderator_id, reason, muted_at, expires_at, unmuted, buyout_allowed)
                   VALUES (?, ?, ?, ?, ?, ?, 0, ?)""",
                (user_id, guild_id, moderator_id, reason, now, expires_at, 1 if buyout_allowed else 0)
            )
            tx.execute(
