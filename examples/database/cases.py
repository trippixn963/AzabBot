...
class CasesMixin:
    ...
    def create_case(
        ...
        logger.tree("Case Created", [
            ...
            ("Action", action_type),
            ("Moderator", resolve(moderator_id)),
        ], emoji="📋")
    ...
    def archive_case(self: "DatabaseManager", case_id: str) -> bool:
        """
        Mark a case as resolved via archival (thread deleted/archived).

        Only updates cases in 'active' or 'expired' status to avoid
        overwriting metadata set by manual resolve (unmute/unban).
        ...
        now = time.time()
        cursor = self.execute(
            """UPDATE cases SET status = 'resolved', resolved_at = COALESCE(resolved_at, ?)
               WHERE case_id = ? AND status IN ('active', 'expired')""",
            (now, case_id)
        )
    ...
    def get_resolved_cases_with_threads(self: "DatabaseManager") -> List[Dict[str, Any]]:
        """
        Get resolved/expired cases older than 15 days that still have thread_ids.

        Uses resolved_at if available, falls back to created_at.

        Returns:
            List of case dicts with non-null thread_ids ready for cleanup.
        """
        cutoff = time.time() - (15 * 86400)  # 15 days ago
        rows = self.fetchall(
            """SELECT * FROM cases
