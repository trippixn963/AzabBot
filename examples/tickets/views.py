...
class BoosterUnjailButton(
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        # Check if buyout is disabled for this mute
        if mute_record and not mute_record.get("buyout_allowed", 1):
            logger.tree("Booster Unjail Blocked", [
                ("User", f"{member.name} ({member.id})"),
                ("Reason", "Buyout disabled"),
            ], emoji="🔒")
            await interaction.followup.send(
                "Buyout is disabled for your mute.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            if hasattr(bot, 'anime_clone') and bot.anime_clone:
                from src.utils.async_utils import create_safe_task
                create_safe_task(
                    bot.anime_clone.destroy_clone(member.id, member.guild),
                    f"AnimeClone-BoosterUnjail-{member.id}",
                )
            ...
            try:
                ...
                if original_message:
                    ...
                    if embed:
                        ...
                        embed.color = EmbedColors.DISCORD_GREEN
