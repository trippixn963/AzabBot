...
from src.utils.mention_resolver import ensure_member_cached
...
class OperationsMixin:
    ...
    async def _cache_members(self: "TicketService", guild: discord.Guild, *users) -> None:
        """Ensure all provided users/IDs are cached so embed mentions render."""
        for user in users:
            if user is not None:
                try:
                    await ensure_member_cached(guild, user)
                except Exception as e:
                    logger.warning(f"Failed to cache member {user}: {e}")

    @property
    def _staff_ping(self: "TicketService") -> Optional[str]:
        """Get the staff role ping string, or None if not configured."""
        if self.config.ticket_staff_role_id:
            return f"<@&{self.config.ticket_staff_role_id}>"
        return None
    ...
    async def create_ticket(
        ...
        # Ensure user is cached so mentions render in embeds
        await self._cache_members(user.guild, user)
        ...
        try:
            ...
            # Ticket staff roles - can view, send, and react
            ...
            # Main ticket staff role (gets pinged)
            if self.config.ticket_staff_role_id:
                staff_role = user.guild.get_role(self.config.ticket_staff_role_id)
                if staff_role:
                    overwrites[staff_role] = staff_perms
                else:
                    logger.warning("Ticket Staff Role Not Found", [
                        ("Role ID", str(self.config.ticket_staff_role_id)),
                        ("Guild", user.guild.name),
                    ])

            # Additional manager roles (no ping)
            if self.config.ticket_manager_role_ids:
                for role_id in self.config.ticket_manager_role_ids:
            ...
            self.db.create_ticket(
                ...
                answers_json=_json.dumps(answers) if answers else None,
                case_id=case_id,
            )
            ...
            # Ping the ticket staff role
            ping_content = self._staff_ping
            if ping_content:
                assigned_text = f"This ticket has been assigned to {ping_content}.\nPlease wait for a staff member to claim your ticket."
            else:
            ...
            if ai_greeting:
                parts = [ai_greeting]
                if wait_time_text:
                    parts.append(f"-# ⏳ {wait_time_text}")
                parts.append("-# Question 1/3")
                await ticket_channel.send(content="\n\n".join(p for p in parts if p))
            else:
                # Fallback if AI is unavailable — ping staff immediately
                fallback = f"Welcome {user.mention}! {assigned_text}"
                if wait_time_text:
                    fallback += f"\n-# ⏳ {wait_time_text}"
                await ticket_channel.send(content=fallback)
                if ping_content:
                    await ticket_channel.send(content=ping_content)
    ...
    async def close_ticket(
        ...
        """
        await self._cache_members(closed_by.guild, closed_by)
        ...
        if channel:
            # Lock channel FIRST to prevent messages during close processing
    ...
    async def claim_ticket(
        ...
        """Claim a ticket."""
        await self._cache_members(staff.guild, staff)
        # Always fetch fresh data for status check (passed ticket may be stale)
        ticket = self.db.get_ticket(ticket_id)
        if not ticket:
            return (False, "Ticket not found.")
        ...
        channel = await self._get_ticket_channel(ticket["thread_id"])
        if channel:
            # Lock out other staff by revoking send on staff roles
            ...
            if self.config.ticket_staff_role_id:
                ...
                if staff_role:
                    try:
                        await channel.set_permissions(staff_role, overwrite=staff_view_only)
                    except discord.HTTPException:
                        pass

            if self.config.ticket_manager_role_ids:
                for role_id in self.config.ticket_manager_role_ids:
                    role = staff.guild.get_role(role_id)
                    if role:
                        try:
                            await channel.set_permissions(role, overwrite=staff_view_only)
                        except discord.HTTPException:
                            pass

            # Grant claimer full permissions (overrides role-level restriction)
            try:
    ...
    async def add_user_to_ticket(
        ...
        await self._cache_members(user.guild, user, added_by)
        # Always fetch fresh data for status check (passed ticket may be stale)
        ticket = self.db.get_ticket(ticket_id)
        if not ticket:
            return (False, "Ticket not found.")
        ...
        await safe_send(channel, content=user.mention, embed=add_embed, view=add_view)
    ...
    async def transfer_ticket(
        ...
        await self._cache_members(new_staff.guild, new_staff, transferred_by)
        # Always fetch fresh data for status check (passed ticket may be stale)
        ticket = self.db.get_ticket(ticket_id)
        if not ticket:
            return (False, "Ticket not found.")
        ...
        channel = await self._get_ticket_channel(ticket["thread_id"])
        if channel:
            # Lock out original claimer — deny send_messages so the role-level
            # staff overwrite doesn't let them keep talking
            if original_claimer_id and original_claimer_id != new_staff.id:
                try:
                    original_member = channel.guild.get_member(original_claimer_id)
                    if original_member:
                        await channel.set_permissions(
                            original_member,
                            view_channel=True,
                        ...
                        logger.tree("Transfer: Original Claimer Locked", [
                            ("Ticket ID", ticket_id),
                            ("Locked", f"{original_member} ({original_member.id})"),
                        ], emoji="🔒")
                except discord.HTTPException as e:
                    log_http_error(e, "Lock Original Claimer Permissions", [
                        ("Ticket ID", ticket_id),
                        ("Original Claimer ID", str(original_claimer_id)),
                    ])
    ...
    async def request_close(
        ...
        await self._cache_members(requester.guild, requester)
        # Always fetch fresh data for status check (passed ticket may be stale)
        ticket = self.db.get_ticket(ticket_id)
        if not ticket:
            return (False, "Ticket not found.")
