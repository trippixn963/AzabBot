...
class PrisonHandler:
    ...
    async def handle_new_prisoner(self, member: discord.Member) -> None:
        ...
        try:
            ...
            # Get prisoner stats, mute record, weekly stats, and gif concurrently
            from src.utils.action_gifs import fetch_action_gif
            prisoner_stats, offense_count_week, time_served_week, gif_url = await asyncio.gather(
                self.bot.db.get_prisoner_stats(member.id),
                asyncio.to_thread(self.bot.db.get_user_mute_count_week, member.id, member.guild.id),
                asyncio.to_thread(self.bot.db.get_user_time_served_week, member.id, member.guild.id),
                fetch_action_gif("mute"),
            )
    ...
    async def handle_prisoner_release(self, member: discord.Member) -> None:
        ...
        try:
            ...
            current_duration, prisoner_stats = await asyncio.gather(
                self.bot.db.get_current_mute_duration(member.id),
                self.bot.db.get_prisoner_stats(member.id),
            )
