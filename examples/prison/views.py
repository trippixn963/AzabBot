...
async def build_appeal_view(
    ...
    buyout_allowed = mute_record.get("buyout_allowed", 1)
    ...
    # Only for mutes >= 1 hour or permanent, and buyout allowed
    # -----------------------------------------------------------------
    if not buyout_allowed:
        logger.tree("Buyout Buttons Skipped", [
            ("User", f"{member.name} ({member.id})"),
