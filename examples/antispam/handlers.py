...
from src.utils.user_cache import resolve
...
class SpamHandlerMixin:
        ...
        if bot.logging_service and bot.logging_service.enabled:
            try:
                embed = discord.Embed(
                    ...
                    timestamp=now)
                embed.add_field(name="User", value=f"{embed_mention(member)}\n{member.id}", inline=True)
    ...
    async def _apply_sticker_spam_mute(
        ...
        try:
            ...
            # Open case
            case_info = await self._open_spam_case(member, "Sticker Spam", mute_duration, violation_count, message.content or "")

            embed = discord.Embed(
                title="🔇 Sticker Spam",
                description=f"{embed_mention(member)} has been muted.",
                color=EmbedColors.WARNING)
            ...
            if bot.logging_service and bot.logging_service.enabled:
                try:
                    log_embed = discord.Embed(
                        ...
                        color=EmbedColors.WARNING,
                        timestamp=now)
                    log_embed.add_field(name="User", value=f"{embed_mention(member)}\n{member.id}", inline=True)
    ...
    async def _send_warning(
        ...
        try:
            embed = discord.Embed(
                title=f"⚠️ {spam_type}",
                description=f"{embed_mention(member)}, your message was deleted.",
                color=EmbedColors.WARNING)
    ...
    async def _apply_mute(
        ...
        try:
            ...
            case_info = await self._open_spam_case(member, spam_type, duration, violation_count, message_content)
    ...
    async def _open_spam_case(
        ...
        """
        Open a case for the spam violation via case_log_service.

        Args:
            member: The member being muted.
            spam_type: Display name of the spam type.
            duration: Mute duration in seconds.
            violation_count: Number of violations.
            message_content: The triggering message content for evidence.

        Returns:
            Case info dict with thread_id, or None on failure.
        """
        bot: "AzabBot" = self.bot  # type: ignore
        ...
        # Build reason and evidence separately
        reason = f"Auto-Spam: {spam_type} (violation #{violation_count})"
        evidence = message_content[:500] if message_content else None
        ...
        try:
            ...
            if case_info:
                logger.tree("Spam Case Log Created", [
            ...
            logger.error_tree("Spam Case Log Failed", e, [
                ("User", f"{member.name} ({member.display_name})"),
                ("ID", str(member.id)),
                ("Type", spam_type),
            ])
        ...
        if bot.logging_service and bot.logging_service.enabled:
            try:
                ...
                if mute_duration > 0:
                    ...
                    embed.add_field(name="User", value=f"{embed_mention(message.author)}\n{message.author.id}", inline=True)
