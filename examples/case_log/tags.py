...
class CaseLogTagsMixin:
    ...
    TAG_AUTOMOD = ("🤖 Automod", discord.Colour.blue())
    TAG_RESOLVED = ("✅ Resolved", discord.Colour.green())
    TAG_EVASION = ("🏃 Evasion", discord.Colour.red())

    ALL_TAGS = [TAG_MUTE, TAG_BAN, TAG_WARN, TAG_FORBID, TAG_AUTOMOD, TAG_RESOLVED, TAG_EVASION]
    ...
    def get_tags_for_case(self: "CaseLogService", action_type: str, is_automod: bool = False) -> List[discord.ForumTag]:
        """
        Get the appropriate tags for a case.

        Args:
            action_type: The action type (mute, ban, warn, forbid).
            is_automod: Whether this is an automated action (adds Automod tag).
        ...
        action_tag_map = {
            ...
            "unmute": self.TAG_RESOLVED[0],
            "unban": self.TAG_RESOLVED[0],
            "unforbid": self.TAG_RESOLVED[0],
            "mute_expired": self.TAG_RESOLVED[0],
            "mute_evasion": self.TAG_EVASION[0],
        }
        ...
        # Add automod tag if this is an automated action
        if is_automod:
            automod_tag = self._tag_cache.get(self.TAG_AUTOMOD[0])
            if automod_tag:
                tags.append(automod_tag)
