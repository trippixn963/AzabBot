...
class CaseLogActionsMixin:
    ...
    async def log_unmute(
        ...
        try:
            ...
            # Add resolved tag while keeping existing tags
            try:
                resolved_tags = self.get_tags_for_case("unmute")
                if resolved_tags:
                    current_tags = list(case_thread.applied_tags)
                    for tag in resolved_tags:
                        if tag not in current_tags:
                            current_tags.append(tag)
                    await case_thread.edit(applied_tags=current_tags[:5])
    ...
    async def log_discussion_ban(
        ...
        """Log a discussion ban — creates a case thread like mute/ban/warn."""
        if not self.enabled:
            return None
        ...
        try:
            case = await self._create_action_case(
                user=target,
                moderator=moderator,
                action_type="discussion_ban",
                reason=f"{scope} — {reason}" if reason else scope,
            )
            ...
            logger.tree("Case Log: Discussion Ban", [
                ("Case ID", case["case_id"]),
            ...
            logger.tree("Discussion Ban Case Log Failed", [
                ("Target", str(target)),
                ("Error", str(e)[:100]),
            ], emoji="❌")
            return None
            ...
            if guild_id:
                ...
                if active_case:
                    ...
                    if case_thread:
                        ...
                        # Add resolved tag while keeping existing tags
                        try:
                            resolved_tags = self.get_tags_for_case("mute_expired")
                            if resolved_tags:
                                current_tags = list(case_thread.applied_tags)
                                for tag in resolved_tags:
                                    if tag not in current_tags:
    ...
    async def log_mute_evasion_return(
        ...
        try:
            ...
            if case_thread:
                ...
                # Add evasion tag
                try:
                    evasion_tags = self.get_tags_for_case("mute_evasion")
                    if evasion_tags:
                        # Keep existing tags and add evasion
                        current_tags = list(case_thread.applied_tags)
                        for tag in evasion_tags:
                            if tag not in current_tags:
                                current_tags.append(tag)
                        await case_thread.edit(applied_tags=current_tags[:5])  # Discord max 5 tags
                except discord.HTTPException:
                    pass
