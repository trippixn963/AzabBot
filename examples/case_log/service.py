...
class CaseLogService(
    ...
    async def _create_action_thread(
        ...
        is_automod = moderator.id == self.bot.user.id if self.bot.user else False
        case_tags = self.get_tags_for_case(action_type, is_automod=is_automod)
    ...
    async def handle_evidence_reply(
        ...
        if not valid_attachments:
            ...
            try:
                ...
                logger.warning(f"Failed to send evidence reminder: {e}")
            return False
        ...
        try:
            ...
            if self.config.assets_channel_id:
                try:
                    assets_channel = self.bot.get_channel(self.config.assets_channel_id)
                    if not assets_channel:
                        assets_channel = await self.bot.fetch_channel(self.config.assets_channel_id)
            ...
            for attachment in valid_attachments:
                ...
                # Upload to assets channel if available (permanent), otherwise case thread
                target_thread = assets_channel if assets_channel else thread
            ...
            if evidence_urls:
                ...
                logger.tree("Evidence Captured", [
                    ...
                    ("Storage", "Assets Channel" if assets_channel else "Case Thread"),
                ], emoji="✅")
    ...
    async def handle_reason_reply(self, message: discord.Message) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to delete warning/reply during reason update: {e}")
